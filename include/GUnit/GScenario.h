...
using step_info_call_map_t =
    std::unordered_map<std::string, step_info_call_step_t>;
...
inline void run(const std::string& feature_file, const std::string& pickles,
  ...
  for (const auto& expected_step : json["steps"]) {
    ...
    for (const auto& given_step : steps) {
      if (detail::match(given_step.first, text)) {
        ...
        const auto line =
            not given_step.second.first.line
                ? expected_step["locations"].back()["line"].get<int>()
...
inline auto call_steps(const TSteps& steps, const std::string& pickles,
                       const std::string& file, detail::type_list<T, Ts...>) {
...
inline void parse_and_register(const std::string& name, const TSteps& steps,
    ...
    if (PatternMatchesString(name.c_str(), feature_name.c_str())) {
      class TestFactory : public internal::TestFactoryBase {
        class test : public Test {
         public:
          test(const TSteps& steps, const std::string& pickles,
          ...
          void TestBody() {
            static_assert(
                std::is_same<Steps,
                             decltype(call_steps(
                                 steps, pickles, file,
                                 detail::function_args_t<TSteps, Steps>{}))>{},
                "STEPS implementation has to return testing::Steps type!");
            call_steps(steps, pickles, file,
                       detail::function_args_t<TSteps, Steps>{});
            std::cout << '\n';
          }
       ...
       public:
        TestFactory(const TSteps& steps, const std::string& pickles,
                    const std::string& file)
            : steps{steps}, pickles{pickles}, file{file} {}
      ...
      MakeAndRegisterTestInfo(
          new TestFactory{steps, pickle, feature},
...
template <class T>
inline auto lexical_table_cast(const std::string& str, const Table&,
                               detail::identity<T>) {
...
class Steps {
 public:
  explicit Steps(const std::string& file, const std::string& scenario)
      : file_{file}, scenario_{scenario} {}

  Steps(const Steps& steps) {
    detail::run(steps.file_, steps.scenario_, steps.before_, steps.steps_,
                steps.after_);
  }
  ...
  auto Given(const TPattern& pattern) {
    ...
    return step<size>{{"Given", File::c_str(), line},
                      pattern.c_str(),
                      steps_[pattern.c_str()]};
  }
  ...
  auto Given(const TPattern& pattern, const T&) {
    constexpr auto size = detail::args_size(TPattern{});
    return step<size, true>{{"Given", File::c_str(), line},
                            pattern.c_str(),
                            steps_[pattern.c_str()]};
  }
  ...
  auto Given(const char* pattern, const T&) {
    return step<-1, true>{
        {"Given", File::c_str(), line}, pattern, steps_[pattern]};
  }

  template <class File = detail::string<>, int line = 0, class TPattern>
  auto When(const TPattern& pattern) {
    constexpr auto size = detail::args_size(TPattern{});
    return step<size>{{"When", File::c_str(), line},
                      pattern.c_str(),
                      steps_[pattern.c_str()]};
  }
  ...
  auto When(const TPattern& pattern, const T&) {
    constexpr auto size = detail::args_size(TPattern{});
    return step<size, true>{{"When", File::c_str(), line},
                            pattern.c_str(),
                            steps_[pattern.c_str()]};
  }
  ...
  auto When(const char* pattern, const T&) {
    return step<-1, true>{
        {"When", File::c_str(), line}, pattern, steps_[pattern]};
  }

  template <class File = detail::string<>, int line = 0, class TPattern>
  auto Then(const TPattern& pattern) {
    constexpr auto size = detail::args_size(TPattern{});
    return step<size>{{"Then", File::c_str(), line},
                      pattern.c_str(),
                      steps_[pattern.c_str()]};
  }
  ...
  auto Then(const TPattern& pattern, const T&) {
    constexpr auto size = detail::args_size(TPattern{});
    return step<size, true>{{"Then", File::c_str(), line},
                            pattern.c_str(),
                            steps_[pattern.c_str()]};
  }
  ...
  auto Then(const char* pattern, const T&) {
    return step<-1, true>{
        {"Then", File::c_str(), line}, pattern, steps_[pattern]};
  }
 ...
 private:
  ...
  class step {
   public:
    ...
    template <class TExpr>
    void operator=(const TExpr& expr) {
      expr_ = {step_info_, [ pattern = pattern_, expr ](const std::string& step,
                                                        const Table& table){
...
private : template <class TExpr, class... Ts>
          static void
          call(const TExpr& expr, const std::string& step, const Table& table,
               const std::string& pattern, detail::type_list<Ts...> t) {
  ...
  call_impl(expr, detail::matches(pattern, step), table, t,
            std::make_index_sequence<sizeof...(Ts)>{},
            std::integral_constant<bool, HasTable>{});
}

template <class TExpr, class TMatches, class... Ts, std::size_t... Ns>
static void call_impl(const TExpr& expr, const TMatches& matches, const Table&,
                      detail::type_list<Ts...>, std::index_sequence<Ns...>,
                      std::false_type) {
  expr(detail::lexical_cast<Ts>(matches[Ns].c_str())...);
}

template <class TExpr, class TMatches, class... Ts, std::size_t... Ns>
static void call_impl(const TExpr& expr, const TMatches& matches,
                      const Table& table, detail::type_list<Ts...>,
                      std::index_sequence<Ns...>, std::true_type) {
  expr(
      detail::lexical_table_cast(Ns < matches.size() ? matches[Ns].c_str() : "",
...
#define STEPS(feature)                                             \
  static __attribute__((unused))::testing::detail::steps<decltype( \
      __GUNIT_CAT(feature, _gtest_string))>                        \
